...
public:
    void init(char* cnf_in); // done
    void create_watch_list(); // done v
    void VSIDS_init(); // done v

    void solve(); // done
    int DPLL(int now_level, int assignment); // done
    void result(char* cnf_name); // done

    // -1 continue, >=0 conflict clause
    int BCP(int assignment, int now_level);
    int first_UIP(vector<int> C, int now_level); // done

    void VSIDS_update(); // done
    int VSIDS_decide(); // done

    void back_to(int level); // done
    void assign(int lit, int val, int level, int c); // done
};
    ...
    while(1){
        ...
        if(end){
            ...
            if(C.size() < 10){
                ...
                if(C.size() >= 1){
                    watched_in_clause[clauses.size() - 1]._f = C[0];
                    if(C[0] > 0) pos_watch[C[0]].push_back(clauses.size() - 1);
                    else neg_watch[abs(C[0])].push_back(clauses.size() - 1);
                }
                if(C.size() >= 2){
                    watched_in_clause[clauses.size() - 1]._s = C[1];
                    if(C[1] > 0) pos_watch[C[1]].push_back(clauses.size() - 1);
                    else neg_watch[abs(C[1])].push_back(clauses.size() - 1);
                }
            }
            int max = 0;
            for(int i = 0; i < C.size(); i++){
...
void solver::solve(){
    while(1){
        ...
        int assignment = VSIDS_decide();
        if(!assignment){
            sat = 1;
            return;
        }
        int result = DPLL(1, assignment);
...
// Recuesively solve it
int solver::DPLL(int now_level, int assignment){
    // Try BCP first
    // return -1: keep going, return >= 0: index of conflict clause
    int conflict = BCP(assignment, now_level);
    if(conflict >= 0){    
        return now_level;
    }
    ...
    // Recurse
    now_level += 1;
    // choose a new lit to assign
    assignment = VSIDS_decide();
    if(!assignment) return -1; // sat

    int result = DPLL(now_level, assignment);
    if(result == -1) return -1;
    if(result < now_level) return result;

    back_to(now_level);
    return DPLL(now_level, assignment*(-1));
}

// go back
void solver::back_to(int level){
